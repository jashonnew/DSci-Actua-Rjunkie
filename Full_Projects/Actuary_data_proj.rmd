---
title: "Tell Me About the Actuaries"
output: 
...
citdat <- left_join(cit, changer, key = AREA)
...
### Actuaries hang out in the big cities
...
### **"Part super-hero. Part fortune-teller. Part trusted advisor. 100% Actuary"** - peraontheissues.com
...
cit$AREA_NAME <- as.factor(cit$AREA_NAME)

cit1.2 <- cit
cit1.2 <- cit[with(cit1.2, order(-TOT_EMP)), ]
```

### **Top Fifteen Metropolitan Areas for Employment**

```{r}
cit1.2 %>% 
  mutate(emprank = rank(TOT_EMP)) %>%
  filter(!is.na(TOT_EMP) & !is.na(AREA_NAME)) %>% 
  #as_factor(AREA_NAME) %>% 
  #fct_recode(AREA_NAME,TOT_EMP)  %>% 
  select(AREA_NAME, TOT_EMP) %>% 
  slice(1:15) %>% 
  kable(col.names = c("Metropolitan Area","Number of Actuaries Employed"))
...
```{r message=FALSE, warning=FALSE, echo=FALSE}
cit1.2 <- cit
cit1.2 <- cit1.2[with(cit1.2, order(-A_MEDIAN)), ]
```

### **Top Fifteen Metropolitan Areas for Median Salary**
...
cit1.2 %>%
  ...
  #as_factor(AREA_NAME) %>%
  #fct_recode(AREA_NAME,TOT_EMP)  %>%
  select(AREA_NAME, A_MEDIAN) %>%
  slice(1:15) %>%
  kable(col.names = c("Metropolitan Area","Median Salary"))
```
